...
CAPABILITY_DESCRIPTIONS = {
    ...
    "matchRecords": "Lookup/match records between ranges using XLOOKUP/VLOOKUP. Params: lookupRange, sourceRange, returnColumns (array of 1-based ints), matchType ('exact'|'approximate'), outputRange, preferFormula (bool, default true)",
    ...
    "addValidation": "Add data validation. Params: range, validationType ('list'|'wholeNumber'|'decimal'|'date'|'textLength'|'custom'), listValues, operator, min, max, formula",
...
def _build_messages(request: PlanRequest) -> list[dict]:
    """
...
async def generate_plan_stream(
    ...
    """
    Stream plan generation.  Yields SSE-ready event dicts:

      {"type": "explanation", "data": "<partial text>"}
      {"type": "plan_ready",  "data": "<JSON plan string>"}
    ...
    async for chunk in response:
        ...
        if delta:
            accumulated += delta
            yield {"type": "explanation", "data": delta}

    try:
        plan_json = extract_json(accumulated)
        _fill_defaults(plan_json, request)
        plan = ExecutionPlan(**plan_json)
        yield {"type": "plan_ready", "data": plan.model_dump_json()}
    except Exception as exc:
        yield {"type": "error", "data": f"Failed to parse plan: {exc}"}
